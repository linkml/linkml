...
class SQLTableGenerator(Generator):
    ...
    * If R is a class, and the slot IS multivalued
        ...
        for cn, c in schema.classes.items():
            ...
            if c.attributes:
                ...
                for uc in c.unique_keys.values():
                    ...
                    is_duplicate = self.check_duplicate_entry_names(autogenerated_item_names, uc_index_name)
                    if not is_duplicate:
            ...
            if not c.abstract or (c.abstract and self.generate_abstract_class_ddl):
                for tag, annotation in c.annotations.items():
                    if tag == "index":
                        index_list = self.parse_annotation_indexes(autogenerated_item_names, annotation)
    ...
    def parse_annotation_indexes(self, autogen: list, ann) -> list:
        index_list = []
        value_dict = {k: annotation for k, annotation in ann.value._items()}
        for key, value in value_dict.items():
            name_exists = self.check_duplicate_entry_names(autogen, key)
            if not name_exists:
