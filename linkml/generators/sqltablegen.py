        ...
        for cn, c in schema.classes.items():
            ...
            if c.attributes:
                ...
                for sn, s in c.attributes.items():
                    ...
                    is_index = fk_index_cond or fk_index_cond
                    col = Column(
                        sql_name(sn),
                        field_type,
                        *args,
                ...
                for uc in c.unique_keys.values():
                    ...
                    cols.append(sql_uc)
                    autogenerated_item_names.append(sql_names[0])
                    # Anything that has a unique constraint should have an associated index with it
                    sql_names[0] = sql_names[0] + "_idx"
                    is_duplicate = self.check_duplicate_entry_names(autogenerated_item_names, cols, sql_names[0])
                    if not is_duplicate:
