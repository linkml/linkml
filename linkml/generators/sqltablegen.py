...
class SQLTableGenerator(Generator):
    ...
    * If R is a class, and the slot IS multivalued
        ...
        for cn, c in schema.classes.items():
            ...
            if c.attributes:
                ...
                for sn, s in c.attributes.items():
                    ...
                    if is_index:
                        autogenerated_item_names.append("ix_" + cn + "_" + sn)
                    if include_comments:
                ...
                for uc in c.unique_keys.values():
                    ...
                    is_duplicate = self.check_duplicate_entry_names(autogenerated_item_names, cols, uc_index_name)
                    if not is_duplicate:
                        sql_names = [uc_index_name] + sql_names
                        uc_index = Index(*sql_names)
                        cols.append(uc_index)
                        autogenerated_item_names.append(uc_index_name)
            if not c.abstract or (c.abstract and self.generate_abstract_class_ddl):
                for tag, annotation in c.annotations.items():
                    if tag == "index":
                        index_list = self.parse_annotation_indexes(autogenerated_item_names, cols, annotation)
                        cols.extend(index_list)
    ...
    # Takes in the existing autogenerated list, the column list, and an annotation to extract the index
    def parse_annotation_indexes(self, autogen: list, cols: list, ann) -> list:
        index_list = []
        value_dict = {k: annotation for k, annotation in ann.value._items()}
        for key, value in value_dict.items():
            name_exists = self.check_duplicate_entry_names(autogen, cols, key)
            if not name_exists:
                annotated_ind = Index(key, *value)
                index_list.append(annotated_ind)
        return index_list

    # Scans the schema for duplicate names that exist as a best practice
    @staticmethod
    def check_duplicate_entry_names(autogen: list, cols_list: list, item_name: str) -> bool:
        for entry in cols_list:
            col_entry_name = getattr(entry, "name", str(entry))
            if isinstance(entry, (Index, UniqueConstraint)):
                if item_name == col_entry_name:
                    exp_list = [getattr(col, "name", str(col)) for col in entry.expressions]
                    msg = (
                        "Warning: proposed item name already exists in Table"
