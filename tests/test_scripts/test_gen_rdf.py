...
repl1: List[Tuple[str, str]] = [
    (
        r'(\s*):generation_date\s*".*"\^\^xsd:dateTime',
        r'\1:generation_date "2019-01-25 12:34"^^xsd:dateTime',
    ),
    (
        r'(\s*):source_file_date\s*".*"\^\^xsd:dateTime',
        r'\1:source_file_date "2019-01-25 12:34"^^xsd:dateTime',
    ),
...
class GenRDFTestCase(ClickTestCase):
    ...
    def test_meta(self):
        ...
        self.do_test(
            f"--context {meta_context_path}",
            "meta.ttl",
            filtr=filtr,
            comparator=ClickTestCase.rdf_comparator,
        )
        self.do_test(
            f"--context {meta_contextn_path} --metauris",
            "metan.ttl",
            filtr=filtr,
            comparator=ClickTestCase.rdf_comparator,
        )
        self.do_test(
            f"-f n3  --context {meta_context_path}",
            "meta.n3",
            filtr=filtr,
            comparator=ClickTestCase.rdf_comparator,
        )
        self.do_test(
            f"-f xsv  --context {meta_context_path}",
            "meta_error",
