...
def test_metamodel(arguments, snapshot_file, snapshot, gen_context_file):
    ...
    runner = CliRunner()
    result = runner.invoke(cli, f"{arguments} --context {gen_context_file} {LOCAL_METAMODEL_YAML_FILE}")
...
def test_make_script(snapshot):
    ...
    runner = CliRunner()
    result = runner.invoke(cli, f"--context {LOCAL_METAMODEL_LDCONTEXT_FILE} {LOCAL_METAMODEL_YAML_FILE}")
    assert result.exit_code == 0
