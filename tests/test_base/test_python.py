...
class PythonTestCase(TestEnvironmentTestCase):
    ...
    def test_types_python(self):
        ...
        env.generate_single_file(
            ...
            lambda: PythonGenerator(
                env.types_yaml, importmap=env.import_map, genmeta=True
            ).serialize(),
            value_is_returned=True,
            filtr=metadata_filter,
            comparator=lambda exp, act: compare_python(
                exp, act, self.env.expected_path("types.py")
            ),
    ...
    def test_mapping_python(self):
        ...
        env.generate_single_file(
            "mappings.py",
            lambda: PythonGenerator(
                env.mapping_yaml, importmap=env.import_map, genmeta=True
            ).serialize(),
            value_is_returned=True,
            filtr=metadata_filter,
            comparator=lambda exp, act: compare_python(
                exp, act, self.env.expected_path("mappings.py")
            ),
            use_testing_root=True,
        )
    ...
    def test_extensions_python(self):
        ...
        env.generate_single_file(
            "extensions.py",
            lambda: PythonGenerator(
                LOCAL_EXTENSIONS_YAML_FILE, importmap=env.import_map, genmeta=True
            ).serialize(),
            value_is_returned=True,
            filtr=metadata_filter,
            comparator=lambda exp, act: compare_python(
                exp, act, self.env.expected_path("extensions.py")
            ),
            use_testing_root=True,
        )
    ...
    def test_annotations_python(self):
        ...
        env.generate_single_file(
            "annotations.py",
            lambda: PythonGenerator(
                LOCAL_ANNOTATIONS_YAML_FILE, importmap=env.import_map, genmeta=True
            ).serialize(),
            value_is_returned=True,
            filtr=metadata_filter,
            comparator=lambda exp, act: compare_python(
    ...
    def test_metamodel_python(self):
        ...
        env.generate_single_file(
            "meta.py",
            lambda: PythonGenerator(
                env.meta_yaml, importmap=env.import_map, genmeta=True
            ).serialize(),
            value_is_returned=True,
            filtr=metadata_filter,
            comparator=lambda exp, act: compare_python(
