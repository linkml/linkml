...
def test_pythongen():
    ...
    assert("Company(id='ROR:1', name=None, aliases=[], ceo=None)" ==str(c))
    h = kitchen_module.EmploymentEvent(employed_at=c.id)
    assert "EmploymentEvent(started_at_time=None, ended_at_time=None, is_current=None, metadata=None, employed_at='ROR:1', type=None)" == str(h)
