...
class SlotUsageTestCase(TestEnvironmentTestCase):
    ...
    def _eval_expected(
        ...
        self.assertEqual(
            usage_slot_name, slot.usage_slot_name
        ) if usage_slot_name else self.assertIsNone(slot.usage_slot_name)
    ...
    def test_multi_usages(self):
        ...
        env.eval_single_file(
            env.expected_path("multi_usages.yaml"), as_yaml(schema), filtr=yaml_filter
        )
    ...
    def test_multi_usages_2(self):
        ...
        env.eval_single_file(
            env.expected_path("multi_usages_2.yaml"), as_yaml(schema), filtr=yaml_filter
        )
