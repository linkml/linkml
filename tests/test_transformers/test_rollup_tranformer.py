...
class TestRollupTransformer(unittest.TestCase):
    ...
    def test_flatten_simple(self):
        ...
        self.assertNotIn("GeneticAssociation", flattened_schema.classes)
        self.assertNotIn("ChemicalAssociation", flattened_schema.classes)
    ...
    def test_multi_level_inheritance(self):
        ...
        self.assertEqual(set(association_class.slots), expected_slots)
        # Verify Entity is still there (not a descendant of Association)
        self.assertIn("Entity", flattened_schema.classes)

    def test_include_all_classes_option(self):
        ...
        self.assertIn("GeneAssociation", flattened_schema.classes)

    def test_mixin_flattening(self):
        ...
        self.assertEqual(set(base_entity_class.slots), expected_slots)

    def test_designator_slot_handling(self):
    ...
    def test_conflicting_slot_ranges(self):
        ...
        flattened_schema = transformer.transform()
        self.assertEqual("integer", flattened_schema.slots["common_slot"].range)
    ...
    def test_union_type_handling(self):
        ...
        association_class = flattened_schema.classes["Association"]
        self.assertIn("special_value", association_class.slots)
        special_value_slot = flattened_schema.slots["special_value"]
        self.assertEqual("string", special_value_slot.range)
        self.assertTrue(special_value_slot.multivalued)
    ...
    def test_indirect_descendants(self):
        ...
        self.assertEqual(expected_slots, set(b_class.slots))

        # A should remain unchanged
        a_class = flattened_schema.classes["A"]
        self.assertEqual(["slot_a"], a_class.slots)

    def test_slot_merging_with_multiple_descendants(self):
        ...
        self.assertEqual(set(["id", "common"]), set(base_class.slots))

        # Common slot should be present only once
        self.assertEqual(1, base_class.slots.count("common"))
