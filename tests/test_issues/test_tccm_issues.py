...
class TCCMTestCase(TestEnvironmentTestCase):
    ...
    def test_references_typeerror(self):
        ...
        SchemaLoader(
            env.input_path("issue_tccm", "resourcedescription.yaml"), mergeimports=False
        ).resolve()
    ...
    def test_slot_usage_only(self):
        ...
        env.generate_single_file(
            ...
            lambda: PythonGenerator(
                env.input_path("issue_tccm", "resourcedescription.yaml"),
    ...
    def test_local_imports(self):
        ...
        env.generate_single_file(
            "importee.py",
            lambda: PythonGenerator(
                env.input_path("issue_tccm", "importee.yaml"),
                importmap=env.import_map,
                mergeimports=False,
            ).serialize(),
            comparator=lambda exp, act: compare_python(
        ...
        env.generate_single_file(
            "importer.py",
            lambda: PythonGenerator(
                env.input_path("issue_tccm", "importer.yaml"),
                importmap=env.import_map,
                mergeimports=False,
            ).serialize(),
            comparator=lambda exp, act: compare_python(
    ...
    def test_minimal_model(self):
        ...
        for generator in Generator.__subclasses__():
            if (
                ...
                env.generate_single_file(
                    ["issue_tccm", "minimalmodel." + generator.valid_formats[0]],
                    lambda: generator(
                        env.input_path("issue_tccm", "minimalmodel.yaml"),
                        importmap=env.import_map,
                        mergeimports=False,
                ...
                env.generate_directory(
                    ["issue_tccm", generator.__name__],
                    lambda d: generator(
                        env.input_path("issue_tccm", "minimalmodel.yaml"),
                        importmap=env.import_map,
                        mergeimports=False,
                        emit_metadata=False,
                    ).serialize(directory=d),
                )
    ...
    def test_dictionary_name(self):
        ...
        yaml = env.generate_single_file(
            ...
            lambda: YAMLGenerator(
                env.input_path("issue_tccm", "explicit_key_id.yaml"),
                importmap=env.import_map,
                mergeimports=False,
                emit_metadata=False,
            ).serialize(),
