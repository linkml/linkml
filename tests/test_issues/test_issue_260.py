    ...
    def test_local_imports(self):
        ...
        env.generate_single_file(
            ...
            lambda: PythonGenerator(
                env.input_path(test_dir, "issue_260a.yaml"),
        ...
        env.generate_single_file(
            fp,
            lambda: PythonGenerator(
                env.input_path(test_dir, "issue_260b.yaml"),
                importmap=env.import_map,
                mergeimports=False,
            ).serialize(),
            comparator=lambda expected, actual: compare_python(
        ...
        env.generate_single_file(
            fp,
            lambda: PythonGenerator(
                env.input_path(test_dir, "issue_260c.yaml"),
                importmap=env.import_map,
                mergeimports=False,
            ).serialize(),
            comparator=lambda expected, actual: compare_python(
        ...
        env.generate_single_file(
            fp,
            lambda: PythonGenerator(
                env.input_path(test_dir, "issue_260.yaml"),
                importmap=env.import_map,
                mergeimports=False,
            ).serialize(),
            comparator=lambda expected, actual: compare_python(
