...
class TestClass(YAMLRoot):
    ...
    class_class_uri: ClassVar[URIRef] = URIRef(
        "https://microbiomedata/schema/TestClass"
    )
    class_class_curie: ClassVar[str] = None
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        if self.test_attribute_1 is not None and not isinstance(
            self.test_attribute_1, str
        ):
            self.test_attribute_1 = str(self.test_attribute_1)

        if self.test_attribute_2 is not None and not isinstance(
            self.test_attribute_2, str
        ):
