# Auto generated from issue_84.yaml by pythongen.py version: 0.0.1
...
class Biosample(YAMLRoot):
    ...
    alternate_identifiers: Optional[
        Union[ElementIdentifier, list[ElementIdentifier]]
    ] = empty_list()
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        ...
        if not isinstance(self.annotations, list):
            self.annotations = (
                [self.annotations] if self.annotations is not None else []
            )
        self.annotations = [
            v if isinstance(v, Annotation) else Annotation(**as_dict(v))
...
class BiosampleProcessing(YAMLRoot):
    ...
    input: Optional[
        Union[
            Union[ElementIdentifier, BiosampleId],
            list[Union[ElementIdentifier, BiosampleId]],
        ]
    ] = empty_list()
    output: Optional[
        Union[
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        ...
        self.input = [
            v if isinstance(v, BiosampleId) else BiosampleId(v) for v in self.input
        ]

        if not isinstance(self.output, list):
            self.output = [self.output] if self.output is not None else []
        self.output = [
            v if isinstance(v, BiosampleId) else BiosampleId(v) for v in self.output
        ]
...
class Annotation(YAMLRoot):
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        ...
        if self.has_characteristic is not None and not isinstance(
            self.has_characteristic, CharacteristicId
        ):
            self.has_characteristic = CharacteristicId(self.has_characteristic)

        if self.has_normalized_value is not None and not isinstance(
...
class ControlledTermValue(NormalizedValue):
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        if self.instance_of is not None and not isinstance(
            self.instance_of, OntologyClassId
        ):
