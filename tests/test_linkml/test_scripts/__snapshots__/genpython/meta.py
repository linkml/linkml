...
class Person(YAMLRoot):
    ...
    addresses: Optional[Union[Union[dict, "Address"], list[Union[dict, "Address"]]]] = (
        empty_list()
    )
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        ...
        self.addresses = [
            v if isinstance(v, Address) else Address(**as_dict(v))
            for v in self.addresses
        ]

        if self.has_birth_event is not None and not isinstance(
            self.has_birth_event, BirthEvent
        ):
        ...
        if self.is_living is not None and not isinstance(
            self.is_living, LifeStatusEnum
        ):
...
class Concept(YAMLRoot):
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        ...
        if self.in_code_system is not None and not isinstance(
            self.in_code_system, CodeSystemId
        ):
...
class Relationship(YAMLRoot):
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        ...
        if self.cordialness is not None and not isinstance(
            self.cordialness, CordialnessEnum
        ):
...
class Dataset(YAMLRoot):
    ...
    companies: Optional[
        Union[
            dict[Union[str, CompanyId], Union[dict, Company]],
            list[Union[dict, Company]],
        ]
    ] = empty_dict()
    activities: Optional[
        Union[
    ...
    code_systems: Optional[
        Union[
            dict[Union[str, CodeSystemId], Union[dict, CodeSystem]],
            list[Union[dict, CodeSystem]],
        ]
    ] = empty_dict()
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        ...
        self._normalize_inlined_as_list(
            slot_name="companies", slot_type=Company, key_name="id", keyed=True
        )
        ...
        self._normalize_inlined_as_list(
            slot_name="code_systems", slot_type=CodeSystem, key_name="id", keyed=True
        )
...
class ClassWithSpaces(YAMLRoot):
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        if self.slot_with_space_1 is not None and not isinstance(
            self.slot_with_space_1, str
        ):
...
class SubclassTest(ClassWithSpaces):
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        if self.slot_with_space_2 is not None and not isinstance(
            self.slot_with_space_2, ClassWithSpaces
        ):
...
class EmploymentEventType(EnumDefinitionImpl):
    ...
    HIRE = PermissibleValue(
        text="HIRE", description="event for a new employee", meaning=BIZCODES["001"]
    )
    FIRE = PermissibleValue(text="FIRE", meaning=BIZCODES["002"])
    PROMOTION = PermissibleValue(
        text="PROMOTION", description="promotion event", meaning=BIZCODES["003"]
    )
    TRANSFER = PermissibleValue(
        text="TRANSFER", description="transfer internally", meaning=BIZCODES["004"]
    )
...
class CordialnessEnum(EnumDefinitionImpl):
    heartfelt = PermissibleValue(
        text="heartfelt", description="warm and hearty friendliness"
    )
    hateful = PermissibleValue(text="hateful", description="spiteful")
    indifferent = PermissibleValue(
        text="indifferent", description="not overly friendly nor obnoxiously spiteful"
    )
