...
class NamedThing(YAMLRoot):
    ...
    class_class_uri: ClassVar[URIRef] = URIRef(
        "http://example.org/example/multi_id/NamedThing"
    )
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        ...
        if self.not_overridden is not None and not isinstance(
            self.not_overridden, IdentifierType
        ):
...
class SequenceVariant(NamedThing):
    ...
    class_class_uri: ClassVar[URIRef] = URIRef(
        "http://example.org/example/multi_id/SequenceVariant"
    )
    class_class_curie: ClassVar[str] = None
