...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            SQLAlchemyGenerator,
            ".sqla.py",
            {"template": TemplateEnum.DECLARATIVE},
            marks=pytest.mark.sqlalchemygen,
        ),
    ],
)
def test_metamodel(generator, extension, serialize_kwargs, temp_dir, snapshot):
    if not extension.startswith("."):
        ...
        generator(LOCAL_METAMODEL_YAML_FILE, directory=str(temp_dir)).serialize(
...
def test_metamodel_shex(format, extension, snapshot):
    ...
    generated = ShExGenerator(LOCAL_METAMODEL_YAML_FILE, format=format).serialize(
        format=format
    )
    assert generated == snapshot(output_file)
