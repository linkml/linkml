...
class TestOwlNormalizePrefixes:
    """OWL generator prefix normalisation tests."""

    def test_sdo_normalised_to_schema(self, tmp_path):
    ...
    def test_flag_off_preserves_original(self, tmp_path):
    ...
    def test_dce_normalised_to_dc(self, tmp_path):
    ...
    def test_custom_prefix_not_affected(self, tmp_path):
    ...
    def test_http_schema_org_normalised(self, tmp_path):
    ...
    def test_no_schema1_from_runtime_http_binding(self, tmp_path):
        ...
        assert not suffixed, (
            f"Auto-generated suffixed prefix(es) {suffixed} found — "
            "runtime http://schema.org/ binding was not cleaned up"
        )
...
class TestShaclNormalizePrefixes:
    """SHACL generator prefix normalisation tests."""
...
class TestContextNormalizePrefixes:
    """JSON-LD context generator prefix normalisation tests (supplements existing tests)."""
...
class TestWellKnownPrefixMap:
    """Tests for the frozen static prefix map."""
    ...
    def test_schema_https(self):
        from linkml.utils.generator import well_known_prefix_map

        wk = well_known_prefix_map()
        assert wk["https://schema.org/"] == "schema"

    def test_schema_http_variant(self):
    ...
    def test_matches_rdflib_defaults(self):
...
class TestCrossGeneratorConsistency:
    """Ensure all generators agree on prefix normalisation."""

    def test_all_generators_normalise_sdo_to_schema(self, tmp_path):
...
class TestPrefixCollision:
    """Collision: user claims the standard prefix name for a different namespace."""

    def test_owl_collision_skips_rename(self, tmp_path, caplog):
        """OWL: myfoaf must NOT be renamed to 'foaf' when user claims that name."""
        from linkml.generators.owlgen import OwlSchemaGenerator
        ...
        with caplog.at_level(logging.WARNING):
            ...
            graph = gen.as_graph()
        bound = {str(p): str(n) for p, n in graph.namespaces()}
        # myfoaf must NOT have been renamed to 'foaf'
    ...
    def test_shacl_collision_skips_rename(self, tmp_path, caplog):
        """SHACL: myfoaf must NOT be renamed to 'foaf' when user claims that name."""
        from linkml.generators.shaclgen import ShaclGenerator
    ...
    def test_context_collision_preserves_user_prefix(self, tmp_path, caplog):
...
class TestJSONLDGeneratorForwarding:
    """Verify JSONLDGenerator propagates flags to its embedded ContextGenerator."""

    def test_normalize_prefixes_forwarded(self, tmp_path):
...
class TestPhase2HttpsPreservation:
    """Phase 2 must not overwrite Phase 1 HTTPS bindings with HTTP variants."""

    def test_phase2_does_not_overwrite_https_with_http(self, tmp_path):
    ...
    def test_normalize_graph_prefixes_phase2_guard(self):
        ...
        assert bound.get("schema") == "https://schema.org/", (
            f"Phase 2 guard failed: schema bound to {bound.get('schema')}"
        )

    def test_empty_schema_no_crash(self, tmp_path):
