...
def test_convert(input_path, cli_runner, tmp_path):
    ...
    schema = input_path("schema_with_inference.yaml")
    data_in = input_path("data_example.yaml")
    json_out = str(tmp_path / "data_example.out.json")
    yaml_out = str(tmp_path / "data_example.out.yaml")
    rdf_out = str(tmp_path / "data_example.out.ttl")
    result = cli_runner.invoke(cli, ["--infer", "-s", schema, data_in, "-o", json_out])
    assert result.exit_code == 0
...
def test_infer_and_convert2(input_path, cli_runner, tmp_path):
    ...
    json_out = str(tmp_path / "data_example2.out.json")
    yaml_out = str(tmp_path / "data_example2.out.yaml")
    rdf_out = str(tmp_path / "data_example2.out.ttl")
    result = cli_runner.invoke(cli, ["--infer", "-s", schema, data_in, "-o", json_out])
    assert result.exit_code == 0
    result = cli_runner.invoke(cli, ["-s", schema, json_out, "-t", "yaml", "-o", yaml_out])
    assert result.exit_code == 0
