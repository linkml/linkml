...
class Strings(YAMLRoot):
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        ...
        if not isinstance(self.mand_multi_string, list):
            self.mand_multi_string = (
                [self.mand_multi_string] if self.mand_multi_string is not None else []
            )
        self.mand_multi_string = [
            v if isinstance(v, str) else str(v) for v in self.mand_multi_string
        ]
        ...
        if not isinstance(self.opt_multi_string, list):
            self.opt_multi_string = (
                [self.opt_multi_string] if self.opt_multi_string is not None else []
            )
        self.opt_multi_string = [
            v if isinstance(v, str) else str(v) for v in self.opt_multi_string
        ]
...
class InheritedStrings2(Strings):
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        if self.req_second_string is not None and not isinstance(
            self.req_second_string, str
        ):
...
class Integers(YAMLRoot):
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        ...
        if not isinstance(self.mand_multi_integer, list):
            self.mand_multi_integer = (
                [self.mand_multi_integer] if self.mand_multi_integer is not None else []
            )
        self.mand_multi_integer = [
            v if isinstance(v, int) else int(v) for v in self.mand_multi_integer
        ]
        ...
        if not isinstance(self.opt_multi_integer, list):
            self.opt_multi_integer = (
                [self.opt_multi_integer] if self.opt_multi_integer is not None else []
            )
        self.opt_multi_integer = [
            v if isinstance(v, int) else int(v) for v in self.opt_multi_integer
        ]
...
class Booleans(YAMLRoot):
    ...
    opt_multi_boolean: Optional[Union[Union[bool, Bool], list[Union[bool, Bool]]]] = (
        empty_list()
    )
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        ...
        if not isinstance(self.mand_multi_boolean, list):
            self.mand_multi_boolean = (
                [self.mand_multi_boolean] if self.mand_multi_boolean is not None else []
            )
        self.mand_multi_boolean = [
            v if isinstance(v, Bool) else Bool(v) for v in self.mand_multi_boolean
        ]
        ...
        if not isinstance(self.opt_multi_boolean, list):
            self.opt_multi_boolean = (
                [self.opt_multi_boolean] if self.opt_multi_boolean is not None else []
            )
        self.opt_multi_boolean = [
            v if isinstance(v, Bool) else Bool(v) for v in self.opt_multi_boolean
        ]
...
class Floats(YAMLRoot):
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        ...
        if not isinstance(self.mand_multi_float, list):
            self.mand_multi_float = (
                [self.mand_multi_float] if self.mand_multi_float is not None else []
            )
        self.mand_multi_float = [
            v if isinstance(v, float) else float(v) for v in self.mand_multi_float
        ]
        ...
        if not isinstance(self.opt_multi_float, list):
            self.opt_multi_float = (
                [self.opt_multi_float] if self.opt_multi_float is not None else []
            )
        self.opt_multi_float = [
            v if isinstance(v, float) else float(v) for v in self.opt_multi_float
        ]
...
class Doubles(YAMLRoot):
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        ...
        if not isinstance(self.mand_multi_double, list):
            self.mand_multi_double = (
                [self.mand_multi_double] if self.mand_multi_double is not None else []
            )
        self.mand_multi_double = [
            v if isinstance(v, float) else float(v) for v in self.mand_multi_double
        ]
        ...
        if not isinstance(self.opt_multi_double, list):
            self.opt_multi_double = (
                [self.opt_multi_double] if self.opt_multi_double is not None else []
            )
        self.opt_multi_double = [
            v if isinstance(v, float) else float(v) for v in self.opt_multi_double
        ]
...
class Times(YAMLRoot):
    ...
    opt_multi_time: Optional[Union[Union[str, XSDTime], list[Union[str, XSDTime]]]] = (
        empty_list()
    )
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        ...
        if not isinstance(self.mand_multi_time, list):
            self.mand_multi_time = (
                [self.mand_multi_time] if self.mand_multi_time is not None else []
            )
        self.mand_multi_time = [
            v if isinstance(v, XSDTime) else XSDTime(v) for v in self.mand_multi_time
        ]
        ...
        if not isinstance(self.opt_multi_time, list):
            self.opt_multi_time = (
                [self.opt_multi_time] if self.opt_multi_time is not None else []
            )
        self.opt_multi_time = [
            v if isinstance(v, XSDTime) else XSDTime(v) for v in self.opt_multi_time
        ]
...
class Dates(YAMLRoot):
    ...
    opt_multi_date: Optional[Union[Union[str, XSDDate], list[Union[str, XSDDate]]]] = (
        empty_list()
    )
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        ...
        if not isinstance(self.mand_multi_date, list):
            self.mand_multi_date = (
                [self.mand_multi_date] if self.mand_multi_date is not None else []
            )
        self.mand_multi_date = [
            v if isinstance(v, XSDDate) else XSDDate(v) for v in self.mand_multi_date
        ]
        ...
        if not isinstance(self.opt_multi_date, list):
            self.opt_multi_date = (
                [self.opt_multi_date] if self.opt_multi_date is not None else []
            )
        self.opt_multi_date = [
            v if isinstance(v, XSDDate) else XSDDate(v) for v in self.opt_multi_date
        ]
...
class DateTimes(YAMLRoot):
    ...
    mand_multi_datetime: Union[
        Union[str, XSDDateTime], list[Union[str, XSDDateTime]]
    ] = None
    opt_datetime: Optional[Union[str, XSDDateTime]] = None
    opt_multi_datetime: Optional[
        Union[Union[str, XSDDateTime], list[Union[str, XSDDateTime]]]
    ] = empty_list()
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        ...
        if not isinstance(self.mand_multi_datetime, list):
            self.mand_multi_datetime = (
                [self.mand_multi_datetime]
                if self.mand_multi_datetime is not None
                else []
            )
        self.mand_multi_datetime = [
            v if isinstance(v, XSDDateTime) else XSDDateTime(v)
            for v in self.mand_multi_datetime
        ]

        if self.opt_datetime is not None and not isinstance(
            self.opt_datetime, XSDDateTime
        ):
            self.opt_datetime = XSDDateTime(self.opt_datetime)

        if not isinstance(self.opt_multi_datetime, list):
            self.opt_multi_datetime = (
                [self.opt_multi_datetime] if self.opt_multi_datetime is not None else []
            )
        self.opt_multi_datetime = [
            v if isinstance(v, XSDDateTime) else XSDDateTime(v)
            for v in self.opt_multi_datetime
        ]
...
class URIorCURIEs(YAMLRoot):
    ...
    mand_multi_uriorcurie: Union[
        Union[str, URIorCURIE], list[Union[str, URIorCURIE]]
    ] = None
    opt_uriorcurie: Optional[Union[str, URIorCURIE]] = None
    opt_multi_uriorcurie: Optional[
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        ...
        if not isinstance(self.mand_multi_uriorcurie, list):
            self.mand_multi_uriorcurie = (
                [self.mand_multi_uriorcurie]
                if self.mand_multi_uriorcurie is not None
                else []
            )
        self.mand_multi_uriorcurie = [
            v if isinstance(v, URIorCURIE) else URIorCURIE(v)
            for v in self.mand_multi_uriorcurie
        ]

        if self.opt_uriorcurie is not None and not isinstance(
            self.opt_uriorcurie, URIorCURIE
        ):
        ...
        if not isinstance(self.opt_multi_uriorcurie, list):
            self.opt_multi_uriorcurie = (
                [self.opt_multi_uriorcurie]
                if self.opt_multi_uriorcurie is not None
                else []
            )
        self.opt_multi_uriorcurie = [
            v if isinstance(v, URIorCURIE) else URIorCURIE(v)
            for v in self.opt_multi_uriorcurie
        ]
...
class URIs(YAMLRoot):
    ...
    opt_multi_uri: Optional[Union[Union[str, URI], list[Union[str, URI]]]] = (
        empty_list()
    )
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        ...
        if not isinstance(self.mand_multi_uri, list):
            self.mand_multi_uri = (
                [self.mand_multi_uri] if self.mand_multi_uri is not None else []
            )
        self.mand_multi_uri = [
            v if isinstance(v, URI) else URI(v) for v in self.mand_multi_uri
        ]
        ...
        if not isinstance(self.opt_multi_uri, list):
            self.opt_multi_uri = (
                [self.opt_multi_uri] if self.opt_multi_uri is not None else []
            )
        self.opt_multi_uri = [
            v if isinstance(v, URI) else URI(v) for v in self.opt_multi_uri
        ]
...
class NCNames(YAMLRoot):
    ...
    opt_multi_ncname: Optional[Union[Union[str, NCName], list[Union[str, NCName]]]] = (
        empty_list()
    )
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        ...
        if not isinstance(self.mand_multi_ncname, list):
            self.mand_multi_ncname = (
                [self.mand_multi_ncname] if self.mand_multi_ncname is not None else []
            )
        self.mand_multi_ncname = [
            v if isinstance(v, NCName) else NCName(v) for v in self.mand_multi_ncname
        ]
        ...
        if not isinstance(self.opt_multi_ncname, list):
            self.opt_multi_ncname = (
                [self.opt_multi_ncname] if self.opt_multi_ncname is not None else []
            )
        self.opt_multi_ncname = [
            v if isinstance(v, NCName) else NCName(v) for v in self.opt_multi_ncname
        ]
...
class ObjectIdentifiers(YAMLRoot):
    ...
    mand_multi_objectidentifier: Union[
        Union[str, ElementIdentifier], list[Union[str, ElementIdentifier]]
    ] = None
    opt_objectidentifier: Optional[Union[str, ElementIdentifier]] = None
    opt_multi_objectidentifier: Optional[
        Union[Union[str, ElementIdentifier], list[Union[str, ElementIdentifier]]]
    ] = empty_list()
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        ...
        if not isinstance(self.mand_multi_objectidentifier, list):
            self.mand_multi_objectidentifier = (
                [self.mand_multi_objectidentifier]
                if self.mand_multi_objectidentifier is not None
                else []
            )
        self.mand_multi_objectidentifier = [
            v if isinstance(v, ElementIdentifier) else ElementIdentifier(v)
            for v in self.mand_multi_objectidentifier
        ]

        if self.opt_objectidentifier is not None and not isinstance(
            self.opt_objectidentifier, ElementIdentifier
        ):
        ...
        if not isinstance(self.opt_multi_objectidentifier, list):
            self.opt_multi_objectidentifier = (
                [self.opt_multi_objectidentifier]
                if self.opt_multi_objectidentifier is not None
                else []
            )
        self.opt_multi_objectidentifier = [
            v if isinstance(v, ElementIdentifier) else ElementIdentifier(v)
            for v in self.opt_multi_objectidentifier
        ]
...
class NodeIdentifiers(YAMLRoot):
    ...
    mand_multi_nodeidentifier: Union[
        Union[str, NodeIdentifier], list[Union[str, NodeIdentifier]]
    ] = None
    opt_nodeidentifier: Optional[Union[str, NodeIdentifier]] = None
    opt_multi_nodeidentifier: Optional[
        Union[Union[str, NodeIdentifier], list[Union[str, NodeIdentifier]]]
    ] = empty_list()
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        ...
        if not isinstance(self.mand_multi_nodeidentifier, list):
            self.mand_multi_nodeidentifier = (
                [self.mand_multi_nodeidentifier]
                if self.mand_multi_nodeidentifier is not None
                else []
            )
        self.mand_multi_nodeidentifier = [
            v if isinstance(v, NodeIdentifier) else NodeIdentifier(v)
            for v in self.mand_multi_nodeidentifier
        ]

        if self.opt_nodeidentifier is not None and not isinstance(
            self.opt_nodeidentifier, NodeIdentifier
        ):
        ...
        if not isinstance(self.opt_multi_nodeidentifier, list):
            self.opt_multi_nodeidentifier = (
                [self.opt_multi_nodeidentifier]
                if self.opt_multi_nodeidentifier is not None
                else []
            )
        self.opt_multi_nodeidentifier = [
            v if isinstance(v, NodeIdentifier) else NodeIdentifier(v)
            for v in self.opt_multi_nodeidentifier
        ]
...
class InheritedTypes(YAMLRoot):
    ...
    mand_multi_InheritedType: Union[
        Union[int, InheritedType], list[Union[int, InheritedType]]
    ] = None
    opt_InheritedType: Optional[Union[int, InheritedType]] = None
    opt_multi_InheritedType: Optional[
        Union[Union[int, InheritedType], list[Union[int, InheritedType]]]
    ] = empty_list()
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        ...
        if not isinstance(self.mand_multi_InheritedType, list):
            self.mand_multi_InheritedType = (
                [self.mand_multi_InheritedType]
                if self.mand_multi_InheritedType is not None
                else []
            )
        self.mand_multi_InheritedType = [
            v if isinstance(v, InheritedType) else InheritedType(v)
            for v in self.mand_multi_InheritedType
        ]

        if self.opt_InheritedType is not None and not isinstance(
            self.opt_InheritedType, InheritedType
        ):
        ...
        if not isinstance(self.opt_multi_InheritedType, list):
            self.opt_multi_InheritedType = (
                [self.opt_multi_InheritedType]
                if self.opt_multi_InheritedType is not None
                else []
            )
        self.opt_multi_InheritedType = [
            v if isinstance(v, InheritedType) else InheritedType(v)
            for v in self.opt_multi_InheritedType
        ]
...
class InheritedType2s(YAMLRoot):
    ...
    mand_multi_InheritedType2: Union[
        Union[str, InheritedType2], list[Union[str, InheritedType2]]
    ] = None
    opt_InheritedType2: Optional[Union[str, InheritedType2]] = None
    opt_multi_InheritedType2: Optional[
        Union[Union[str, InheritedType2], list[Union[str, InheritedType2]]]
    ] = empty_list()
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        ...
        if not isinstance(self.mand_multi_InheritedType2, list):
            self.mand_multi_InheritedType2 = (
                [self.mand_multi_InheritedType2]
                if self.mand_multi_InheritedType2 is not None
                else []
            )
        self.mand_multi_InheritedType2 = [
            v if isinstance(v, InheritedType2) else InheritedType2(v)
            for v in self.mand_multi_InheritedType2
        ]

        if self.opt_InheritedType2 is not None and not isinstance(
            self.opt_InheritedType2, InheritedType2
        ):
        ...
        if not isinstance(self.opt_multi_InheritedType2, list):
            self.opt_multi_InheritedType2 = (
                [self.opt_multi_InheritedType2]
                if self.opt_multi_InheritedType2 is not None
                else []
            )
        self.opt_multi_InheritedType2 = [
            v if isinstance(v, InheritedType2) else InheritedType2(v)
            for v in self.opt_multi_InheritedType2
        ]
...
class InheritedType3s(YAMLRoot):
    ...
    mand_multi_InheritedType3: Union[
        Union[str, InheritedType3], list[Union[str, InheritedType3]]
    ] = None
    opt_InheritedType3: Optional[Union[str, InheritedType3]] = None
    opt_multi_InheritedType3: Optional[
        Union[Union[str, InheritedType3], list[Union[str, InheritedType3]]]
    ] = empty_list()
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        ...
        if not isinstance(self.mand_multi_InheritedType3, list):
            self.mand_multi_InheritedType3 = (
                [self.mand_multi_InheritedType3]
                if self.mand_multi_InheritedType3 is not None
                else []
            )
        self.mand_multi_InheritedType3 = [
            v if isinstance(v, InheritedType3) else InheritedType3(v)
            for v in self.mand_multi_InheritedType3
        ]

        if self.opt_InheritedType3 is not None and not isinstance(
            self.opt_InheritedType3, InheritedType3
        ):
        ...
        if not isinstance(self.opt_multi_InheritedType3, list):
            self.opt_multi_InheritedType3 = (
                [self.opt_multi_InheritedType3]
                if self.opt_multi_InheritedType3 is not None
                else []
            )
        self.opt_multi_InheritedType3 = [
            v if isinstance(v, InheritedType3) else InheritedType3(v)
            for v in self.opt_multi_InheritedType3
        ]
