...
* PR state labels — All open, non-draft PRs must carry exactly one of the following labels at all times: `awaiting-review`, `awaiting-author`, `in-review`, or `on-hold`. These labels are the authoritative signal of who holds the ball. When a reviewer requests changes, they apply `awaiting-author`. When an author has addressed feedback, they switch it to `awaiting-review`. A PR with `awaiting-author` that receives no activity for 3 months it will be automatically marked stale; after a further 14 days it will be moved to draft and marked orphaned.
