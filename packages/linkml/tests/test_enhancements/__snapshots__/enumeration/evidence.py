...
class Evidence(EnumDefinitionImpl):
    ...
    IEA = PermissibleValue(text="IEA", description="Colonel Mustard in the Ballroom")
    ISS = PermissibleValue(text="ISS", description="Mrs. Peacock with the Dagger", meaning=CLUE["1173"])
...
slots.evidencer__name = Slot(
    uri=EVIDENCE.name,
    name="evidencer__name",
    curie=EVIDENCE.curie("name"),
    model_uri=EVIDENCE.evidencer__name,
    domain=None,
    range=URIRef,
)

slots.evidencer__code = Slot(
    uri=EVIDENCE.code,
    name="evidencer__code",
    curie=EVIDENCE.curie("code"),
    model_uri=EVIDENCE.evidencer__code,
    domain=None,
    range=Union[str, "Evidence"],
)
