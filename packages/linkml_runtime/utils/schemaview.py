...
def detect_cycles(f: Callable[[Any], Iterable[Any] | None], x: Any) -> None:
    ...
    Uses the classic white/grey/black colour coding algorithm to track which nodes have been explored.
    ...
    A directed cycle reachable from node `x` raises a CycleError.
    ...
    # processed_flag == True means all neighbours (nodes generated by running
    # `f(node)`) have been added to the todo stack and the node can be marked BLACK.
